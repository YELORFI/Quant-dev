...
Model_with_vol::Model_with_vol(const double& risk_free_rate, const double& correlation, const Pair& init_spot_variance, const double& kappa, const double& vol_of_vol, const double& theta, const DupireLocalVolatilitySurface& dupirelocalvolatility,const bins&b) 
	:_risk_free_rate(risk_free_rate), _correlation(correlation),_init_spot_variance(init_spot_variance),_kappa(kappa),_vol_of_vol(vol_of_vol),_theta(theta),_dupire_local_volatility(dupirelocalvolatility),bin(b.clone())
{
}
...
Model_with_vol::Model_with_vol(const Model_with_vol& model)
	:_risk_free_rate(model._risk_free_rate),_correlation(model._correlation),_init_spot_variance(model._init_spot_variance),_kappa(model._kappa),_vol_of_vol(model._vol_of_vol),_theta(model._theta),_dupire_local_volatility(model._dupire_local_volatility),_epsilon(model._epsilon),bin(model.bin->clone())
{
}
...
double Heston_local_sto_vol_Model::der_psi_function(const double& variance) const
{
	double variance_up = psi_function(variance + _epsilon);
	double variance_down = psi_function(variance - _epsilon);
	double deriv = 0.5 * (variance_up - variance_down) / (_epsilon);
	return deriv;
}
...
double Model_with_vol::epsilon() const
{
    return _epsilon;
}
