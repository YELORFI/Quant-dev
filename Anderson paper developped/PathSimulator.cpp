...
PathSimulator::PathSimulator(const Model_with_vol& model, const Vector time_points) 
	:model(model.clone()), _time_points(time_points)
{
}
...
PathSimulatorEuler* PathSimulatorEuler::clone() const
{
	return new PathSimulatorEuler(*this);
}

PathSimulatorEuler::PathSimulatorEuler(const Heston_local_sto_vol_Model& model, const Vector time_points) :PathSimulator(model, time_points)
{
}
...
PathSimulatorMilstein* PathSimulatorMilstein::clone() const
{
	return new PathSimulatorMilstein(*this);
}

PathSimulatorMilstein::PathSimulatorMilstein(const Heston_local_sto_vol_Model& model, const Vector time_points) :PathSimulator(model, time_points)
{
}

Pair PathSimulatorMilstein::next_step(const size_t& time_idx, const Pair& spot_variance, vector<Pair>& spot_variance_sample) const
{
	if (time_idx == 0)
		return model->init_spot_variance();
	...
	double z3 = model->correlation() * z1 + sqrt(1 - pow(model->correlation(), 2)) * z2;
	//Gives the volatility discretization 
	double drift_variance = model->variance_drift(_time_points[time_idx - 1], max(spot_variance.second, 0.0));
	double diffusion_variance = model->variance_diffusion(_time_points[time_idx - 1], max(spot_variance.second, 0.0));
	next2 = spot_variance.second + drift_variance * delta_t + diffusion_variance * sqrt(delta_t) * z3 + 0.5 * diffusion_variance * drift_variance * (pow(z3, 2) - 1) * delta_t;
	// To avoid negative values of the variance (Full Truncation)
	next2 = max(next2, 0.0);
	// Gives the asset discretization
	if (time_idx == 1)
	{
		next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + 
		spot_variance.first * model->psi_function(max(spot_variance.second, 0.0)) * sqrt(delta_t) * z1;
		return Pair(max(next1,10e-100), next2);
	}
	...
	double local_vol = sqrt(model->local_volatility(_time_points[time_idx - 1], spot_variance_sample,
		 spot_variance.first));
	next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + 
		local_vol * spot_variance.first * model->psi_function(max(spot_variance.second, 0.0)) * 
		sqrt(delta_t) * z1 + 0.5 * local_vol * spot_variance.first * 
		model->der_psi_function(max(spot_variance.second, 0.0)) * diffusion_variance * 
		(z3 * delta_t + pow(z3, 2) * sqrt(delta_t));
	return Pair(max(next1, 10e-100), next2);
}
...
PathSimulatorQE::PathSimulatorQE(const Model_with_vol& model, const Vector time_points) :PathSimulator(model, time_points)
{
}
...
Pair PathSimulatorQE::next_step(const size_t& time_idx, const Pair& spot_variance, vector<Pair>& spot_variance_sample) const
{
	...
	if (psi <= 1.5)
	{
		...
		double z_v = NormalCDFInverse(u_v); // \phi^-1 ---> Moro algo
		next2 = a * pow(b + z_v, 2);
	}
	else
	{
...
make sure Milstein and euler and QE work properly (minor mistakes)



PathSimulatorTG* PathSimulatorTG::clone() const


PathSimulatorTG::PathSimulatorTG(const Model_with_vol& model, const Vector time_points) :PathSimulator(model, time_points)


Pair PathSimulatorTG::next_step(const size_t& time_idx, const Pair& spot_variance, vector<Pair>& spot_variance_sample) const
